#!/bin/bash
#This script will make a set of folders and files that are set-up in a manner
#that is conducive to be used with our CMake infrastructure.

if [[ $# -ne 1 ]]; then
    echo "Syntax: BasicSetup.sh <Name of Resulting Library>"
    exit 1
fi

LIBRARY_NAME=$1
#This will be the name of the directory for the source code
SRC_DIR=${LIBRARY_NAME}
#This will be the name of the directory for test's source code
TEST_DIR=${LIBRARY_NAME}_Test
#Defines paths for resulting CMakeLists.txt files
...
echo "">>.gitignore
echo "#These are common temp file names"
echo "*.swp"
